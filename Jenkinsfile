pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE = "duongtrong27/booking_be:latest" // Thay bằng tên image Docker Hub của bạn
    }
    stages {
        stage('Build Docker Image') {
            steps {
                script {
                    // Build Docker Image
                    sh '''
                    sudo docker build -t booking_be:latest .
                    '''
                }
            }
        }
        stage('Tag Docker Image') {
            steps {
                script {
                    // Sửa tag cho image
                    sh '''
                    sudo docker tag booking_be:latest $DOCKER_IMAGE
                    '''
                }
            }
        }
